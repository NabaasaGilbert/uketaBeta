  ...
  try {
    ...
    if (checkExistance.length >= 1) {
      if (data.progress >= checkExistance[0].progress) {
        const updateProgress = await prisma.lectureProgress.update({
      ...
      const newProgress = await prisma.lectureProgress.create({
        data: {
          User: {
            connect: {
              email: data.userEmail,
    ...
    if (data.progress === "Completed") {
      ...
      if (checkCourseExistance.length >= 1) {
        ...
        const updateProgress = await prisma.courseProgress.update({
        ...
        if (lectureCount === completedLectureCount) {
          const emailSubject = `🎊🥳Thank and Congratulations for Completing Your UKETA Learning Course🎊🥳`;
